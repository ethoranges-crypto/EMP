...
/**
 * SPEC §4.2: protocol onboarding is just "connect wallet + SIWE" — no Safe
 * option (that's a user-side thing, §4.1). The signature proves ownership
 * of the wallet an admin will later cross-reference against the protocol's
 * out-of-band proof (e.g. a DM confirming that same address).
 */
export function SignInPanel({ onSignedIn }: { onSignedIn: () => void }) {
  ...
  async function handleClick() {
    ...
    const result = await signIn({ address, chainId, role: "protocol", accountType: "EOA" });
