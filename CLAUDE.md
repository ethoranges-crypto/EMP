...
4. **No hardcoded secrets.** Bot token, RPC keys, treasury addresses, DB creds → env/secret store only. Treasury address is deliberately env-only rather than admin/DB-configurable, unlike other business config — see Payments below for why.
...
- **One EMP treasury address per chain** — never per-campaign wallets. Env-only (`{CHAIN}_TREASURY_ADDRESS`), not admin/DB-configurable: it's the single highest-value config in the system — redirect it and every protocol payment reroutes to whoever made the change — so changing it requires server/deploy access, not just an admin session (which is reachable by anyone who compromises an admin wallet/session). The admin panel (`/api/admin/treasury`) may only *display* it, never edit it. Both the Pay panel and the worker's payment watcher read the exact same env-sourced value (`getPayableChains()` in `@emp/config`) — no separate DB copy that could drift. A chain is only "payable" (offered to protocols, watched by the worker) once it has both an RPC URL and a treasury address.
- Picking a chain/token immediately opens the payment window: creates the `Payment` row and moves the campaign `APPROVED → AWAITING_PAYMENT` (see `setCampaignPaymentMethod`).
- MVP verification is **fully automated, no manual step in the default path**: apps/worker's `EvmTreasuryWatcher` polls the chain for the expected token + amount from the protocol's authenticated wallet within the payment window, and flips the campaign to `SENDING` itself once verified. A manual "paste tx hash, admin verifies" fallback is deliberately not built — only add it if automated detection proves impractical in practice.
