...
4. **No hardcoded secrets.** Bot token, RPC keys, DB creds → env/secret store only. (Treasury addresses are the deliberate exception — see Payments below: they're admin-configurable via the DB, not env, because they need to change without a deploy.)
...
- **One EMP treasury address per chain** — never per-campaign wallets. Admin-configurable via the DB (`ChainTreasury`, `/api/admin/treasury`), not env — a chain's RPC URL stays env-only (can embed a provider secret), but its treasury address is ordinary business config an admin should be able to change without a deploy. A chain is only "payable" (offered to protocols, watched by the worker) once it has both.
